...
void* edges(argument_h *args) {
  ...
  for(i=args->start;i<n_pixels;i = i +args->stride) {
  y = i / width;
  x = i - (width * y);
  //for(i=args->start;i<n_pixels;i = i +args->stride) {
    if (x == 0 || y == 0 || x == width - 1 || y == height - 1) {
        ...
        args->result = (image[i] * 4) + (image[b] * -1) + (image[d] * -1) + (image[f] * -1)
            + (image[h] * -1);

        if (args->result > 0) { // if the result is positive this is an edge pixel
          results[i] = 255;
        } else {
          results[i] = 0;
        }
...
void* detect_edges(unsigned char *in, unsigned char *out) {
  ...
  argument_h l[n_threads];

  for(i=0;i<n_threads;i++){
     l[i].start = i;
     l[i].stride = n_threads;
     l[i].result = 0;
  }
    
  for(i=0;i<n_threads;i++){
      pthread_create(&t[i], NULL, edges, &l[i]);
  }
