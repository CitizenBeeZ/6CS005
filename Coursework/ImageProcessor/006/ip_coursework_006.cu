...
__global__ void kernel(unsigned char *in, unsigned char *out) {
...
int main(int argc, char **argv) {
  ...
  cudaMalloc((void**) &d_results, sizeof(unsigned char) * n_pixels);
  cudaMalloc((void**) &d_image, sizeof(unsigned char) * n_pixels);
  
  cudaMemcpy(d_image, image, (sizeof(unsigned char) * 7200), cudaMemcpyHostToDevice);
  
  kernel <<<100, 72>>>(d_image, d_results);

  cudaMemcpy(results, d_results, (sizeof(unsigned char) * n_pixels), cudaMemcpyDeviceToHost);
